...
import 'package:moodiary/feature/sync/data/codec.dart';
import 'package:moodiary/feature/sync/data/incremental_engine.dart';
...
void main() {
  ...
  Uint8List jsonBytes(Object v) => Uint8List.fromList(utf8.encode(jsonEncode(v)));
  ...
  group('push — first sync', () {
    test('uploads all diaries + categories and writes a v4 manifest', () async {
      ...
      final report = await engineOn(backend, diaries: store, categories: cats)
          .push();
  ...
  group('push — last-writer-wins (millisecond int)', () {
    test('skips re-upload when local is not newer than remote', () async {
      ...
      await engineOn(backend, diaries: store).push();
      final firstWrites = backend.opCount('write', SyncKeys.diaryObjectPath('a'));

      // 同一份本地（时间戳不变）再 push 一次 → 不应重传。
      final report = await engineOn(backend, diaries: store).push();
      expect(report.diaryCount, 0);
      expect(backend.opCount('write', SyncKeys.diaryObjectPath('a')), firstWrites);
    });
  ...
  group('push — media', () {
    test('uploads media BEFORE the diary JSON and records confirmed refs',
        () async {
      final backend = FakeRemoteBackend();
      ...
      final jsonWrite = backend.ops.indexOf('write ${SyncKeys.diaryObjectPath('a')}');
      expect(mediaWrite, greaterThanOrEqualTo(0));
      expect(jsonWrite, greaterThan(mediaWrite),
          reason: '媒体必须先于 diary JSON 上传');
      expect(backend.manifest()!.entries['d:a']!.media, ['image/img-1.jpg']);
    });

    test('locally-missing media is skipped, not claimed in manifest, no failure',
        () async {
      final backend = FakeRemoteBackend();
    ...
    test('a real media upload failure aborts the diary JSON write (no broken ref)',
        () async {
      final backend = FakeRemoteBackend();
      final media = FakeMediaFiles()..put('image', 'img-1.jpg');
      ...
      final report = await engineOn(backend, diaries: store, media: media).push();
      expect(report.failed, 1);
      // 媒体上传失败 → diary JSON 不得写入（避免远端 JSON 引用到未上传的媒体）。
      expect(backend.hasObject(SyncKeys.diaryObjectPath('a')), isFalse);
      expect(backend.manifest()?.entries.containsKey('d:a') ?? false, isFalse);
      // 有失败条目 → 不推进上次同步时间。
      expect(MoodiaryKVs.lastSyncTime.get(), 0);
    });

    test('deferred stale-media delete spares media re-referenced by another diary',
        () async {
      ...
      await seedRemote(
        backend,
        diaries: [buildDiary(id: 'a', modifiedMs: 100, images: ['R.jpg'])],
        media: media,
      );
      ...
      await engineOn(backend, diaries: local, media: media, concurrency: 1).push();
    ...
    test('removes remote media no longer referenced after an edit', () async {
      ...
      expect(backend.hasObject('media/image/old.jpg'), isFalse,
  ...
  group('push — partial failure', () {
    test('one failed diary does not block others and does not advance sync time',
        () async {
      final backend = FakeRemoteBackend();
      backend.beforeOp = (op, key) {
  ...
  group('pull — first sync', () {
    test('downloads remote diaries, categories and media into empty local',
        () async {
      final backend = FakeRemoteBackend();
      final remoteMedia = FakeMediaFiles()..put('image', 'p.jpg');
      await seedRemote(
        backend,
        diaries: [buildDiary(id: 'a', modifiedMs: 100, images: ['p.jpg'])],
  ...
  group('pull — last-writer-wins', () {
    test('does not overwrite a newer local edit with an older remote', () async {
      final backend = FakeRemoteBackend();
      await seedRemote(backend, diaries: [buildDiary(id: 'a', modifiedMs: 1000, title: 'remote')]);
    ...
    test('downloads when remote is newer than local', () async {
      final backend = FakeRemoteBackend();
      await seedRemote(backend, diaries: [buildDiary(id: 'a', modifiedMs: 2000, title: 'remote-newer')]);

      final local = FakeDiaryStore([buildDiary(id: 'a', modifiedMs: 1000, title: 'local')]);
      final report = await engineOn(backend, diaries: local).pull();
      expect(report.diaryCount, 1);
      expect(local.diaries['a']!.title, 'remote-newer');
    });

    test('backfills missing media on an otherwise-skipped (equal) entry', () async {
      final backend = FakeRemoteBackend();
      final remoteMedia = FakeMediaFiles()..put('image', 'p.jpg');
      await seedRemote(
        backend,
        diaries: [buildDiary(id: 'a', modifiedMs: 1000, images: ['p.jpg'])],
        media: remoteMedia,
      );
      ...
      final localMedia = FakeMediaFiles();
      final report = await engineOn(backend, diaries: local, media: localMedia).pull();
  ...
  group('pull — tombstone', () {
    ...
    test('keeps a local edit that is newer than the remote tombstone (fix [2])',
        () async {
      final backend = FakeRemoteBackend();
      backend.objects[SyncKeys.manifestPath] = jsonBytes({
        'version': 4,
  ...
  group('sync — pull then push in one critical section', () {
    test('merges remote-only and local-only entries', () async {
      final backend = FakeRemoteBackend();
      await seedRemote(backend, diaries: [buildDiary(id: 'remote', modifiedMs: 100)]);
  ...
  group('cancellation', () {
    test('a requested stop skips remaining items and does not advance sync time',
        () async {
      final backend = FakeRemoteBackend();
      final store = FakeDiaryStore([
  ...
  group('multi-backend tombstone (never lose a deletion before all backends)', () {
    test('hard-deletes only after EVERY configured backend received the tombstone',
        () async {
      // 两个云后端都已配置。
      await configureBackend(SyncProviderType.webdav);
  ...
  group('regression — corrupt remote manifest must not rebuild from zero (fix [1])',
      () {
    test('a present-but-non-object manifest aborts push instead of dropping entries',
        () async {
      final backend = FakeRemoteBackend();
  ...
  group('regression — stale local deletion must not clobber newer remote (fix [5])',
      () {
    test('tombstone push skips when the remote entry is newer', () async {
      final backend = FakeRemoteBackend();
      // 远端有 a 的较新普通条目（来自别的设备的更新）。
      await seedRemote(backend, diaries: [buildDiary(id: 'a', modifiedMs: 2000, title: 'newer-remote')]);
      expect(backend.hasObject(SyncKeys.diaryObjectPath('a')), isTrue);
  ...
  group('regression — concurrent manifest clobber aborts cleanly (lease CAS)', () {
    ...
    test('write-then-readback token mismatch → throw, no remote/local destruction',
        () async {
      final backend = FakeRemoteBackend();
      // 远端已有普通条目 a + 其媒体。
      final media = FakeMediaFiles()..put('image', 'p.jpg');
    ...
    test('normal push succeeds when readback token matches (deferred deletes run)',
        () async {
      final backend = FakeRemoteBackend();
      final media = FakeMediaFiles()
        ..put('image', 'old.jpg')
      ...
      await seedRemote(
        backend,
        diaries: [buildDiary(id: 'a', modifiedMs: 100, images: ['old.jpg'])],
        media: media,
      );
  ...
  group('re-audit fixes', () {
    test('category tombstone push skips when the remote category is newer (A)',
        () async {
      final backend = FakeRemoteBackend();
      await seedRemote(
        backend,
        cats: [buildCategory(id: 'c1', modifiedMs: 2000, name: 'newer-remote')],
      );
      expect(backend.hasObject(SyncKeys.categoryObjectPath('c1')), isTrue);
    ...
    test('pull counts a category local-write failure (no false success) (B)',
        () async {
      final backend = FakeRemoteBackend();
      await seedRemote(backend, cats: [buildCategory(id: 'c1', modifiedMs: 1000)]);
    ...
    test('corrupt entries field (non-Map) aborts push, no manifest rebuild (C)',
        () async {
      final backend = FakeRemoteBackend();
      backend.objects[SyncKeys.manifestPath] = jsonBytes({
        'version': 4,
    ...
    test('emits a syncEnd even when push throws (auto-sync gate safety) (D)',
        () async {
      final backend = FakeRemoteBackend();
      ...
      await expectLater(
        engineOn(backend, diaries: FakeDiaryStore([buildDiary(id: 'a', modifiedMs: 1)]))
            .push(),
        throwsA(isA<SyncException>()),
      );
