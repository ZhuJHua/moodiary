...
/// 工具执行体：接收已解析入参，返回回灌给模型的文本结果。**不含权限校验**
/// （权限闸门由 [AssistantService] 的 toolDispatch 回调包裹）。
...
/// 助手内置工具的共享注册表（日记 + 分类全 CRUD），顺序与 [AssistantTool.values] 一致。
abstract final class AssistantToolRegistry {
  ...
  static const List<AssistantToolSpec> specs = [
    AssistantToolSpec(
      ...
      jsonSchema: {
        ...
        'properties': {
          'query': {
            'type': 'string',
            'description': '空格分隔的检索关键词，例如 "旅行 海边"。',
          },
        },
        'required': ['query'],
      },
    ...
    AssistantToolSpec(
      ...
      jsonSchema: {
        ...
        'properties': {
          ...
          'content': {
            'type': 'string',
            'description': '日记正文，支持 Markdown 格式。',
          },
          'mood': {
            'type': 'number',
    ...
    AssistantToolSpec(
      ...
      jsonSchema: {
        'type': 'object',
        'properties': {
          'id': {'type': 'string', 'description': '目标日记的 id（来自 searchDiaries）。'},
    ...
    AssistantToolSpec(
      ...
      jsonSchema: {
        'type': 'object',
        'properties': {
          'id': {'type': 'string', 'description': '目标分类的 id（来自 listCategories）。'},
  ...
  /// 按 function name 查规格；未注册返回 null。
  ...
  static Future<String> _createDiary(Map<String, dynamic> input) async {
    ...
    final diary = Diary.create(
      ...
      // 助手生成 Markdown，落库统一转 tiptap JSON；contentText 为纯文本（卡片 / 搜索用）。
  ...
  /// 助手生成的 markdown → tiptap 落库形态（纯 Dart [MarkdownToTiptap] 转 JSON）；
  /// 失败回退为 markdown（只读旧格式），保证至少能创建成功。
  ...
  static Future<String> _deleteDiary(Map<String, dynamic> input) async {
    ...
    final title = existing.title.trim().isEmpty ? '(无标题)' : existing.title.trim();
  ...
  static Future<String> _deleteCategory(Map<String, dynamic> input) async {
    ...
    return ok
        ? '已删除分类（id=$id）。'
        : '删除失败：分类不存在，或其下仍有日记（请先移除 / 改归类后再删）。';
  }

  /// 解析并校验 categoryId：为空或对应分类不存在 / 已删除时返回 null。
  ...
  /// 把 mood 入参夹紧到 0.0~1.0；非数值返回 null。
