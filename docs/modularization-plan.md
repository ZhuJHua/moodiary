...
> - **Phase 6a（媒体库 → 新 feature 包）✅** — 抽出 **`moodiary_media`**（feature 层）：`MediaPage` + `MediaDiaries`/`MediaCleanupController`（生成文件 move-as-is）+ 图片/视频查看器。零跨 feature 依赖、唯一消费者 `root_shell`（app 层），干净利落（同 assistant 范式）；provider 名不变，`root_shell` 照常解析。桌面端可直接复用媒体库。baseline 仍 5（media 本就不在违规边上）。**Phase 6 待续**：`moodiary_editor_host`（编辑器 host 接线，较难——涉及 edit 内部 + diary 详情耦合）。
