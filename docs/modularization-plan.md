...
> - **Phase 5b（diary/category 控制器 → data）✅** — 把 `diary_controller`（`DiaryController`/`RecycleBinDiaries`/`getDiary`）与 `category_controller`（`CategoryController`/`getCategory`/`categoryById`）连同 `.g.dart` **整体 `git mv` 进 `moodiary_data`**（两文件纯仓储驱动、无 feature 依赖，故 move-as-is、**免 build_runner**；仅把包内 barrel 自引改为具体仓储文件、`getCategory` 的 `firstWhereOrNull` 换手写循环以免引入 `collection`）。`moodiary_data` 加 `flutter_riverpod`+`riverpod_annotation`（已在 lock 内）。12 个消费者改依赖 data barrel；`category_by_id_test` 迁入 data 包测试。**data 层现拥有 diary/category 响应式状态**（合法的「data 拥有仓储+状态」模式，略偏计划的「只搬读 provider」但被耦合所迫、且免了 build_runner）。清 `edit_controller→diary`、`category_picker_sheet→diary`，**baseline 7→5**。
>   - **Phase 5 待续**：`EditorMigrationService`/`DashboardStats`/geo-weather → `moodiary_data`。
> - **Phase 6a（媒体库 → 新 feature 包）✅** — 抽出 **`moodiary_media`**（feature 层）：`MediaPage` + `MediaDiaries`/`MediaCleanupController`（生成文件 move-as-is）+ 图片/视频查看器。零跨 feature 依赖、唯一消费者 `root_shell`（app 层），干净利落（同 assistant 范式）；provider 名不变，`root_shell` 照常解析。桌面端可直接复用媒体库。baseline 仍 5（media 本就不在违规边上）。
> - **Phase 6b（编辑子系统 → 新 feature 包）✅** — Phase 5b 清掉 `edit→diary` 后，**整个 `feature/edit` 已自洽**（零跨 feature 依赖），故把 17 个文件（编辑器 host/body/toolbar、自动保存 `EditController`、分类选择、地理/天气采集+模型、录音 sheet，含 5 个生成文件）连同 `component/quill_embed`（唯一消费者是 `editor_body`）整体 `git mv` 进 **`moodiary_editor_host`**（内部 import 机械换前缀，move-as-is 免 build_runner）。3 个消费者（`migration_compare`/`detail_diary_page`/`diary_page`）改依赖包 barrel。清掉 `detail→edit`、`diary_page→edit`、`migration_compare→edit` 3 条边。**baseline 5→2** —— 剩下 `diary_page→sync`、`setting→lock` 正是计划刻意保留的 app 侧内嵌。`component/` 层就此清空（README 一并删）。
> - **模块化主体至此基本完成**：baseline **27→2**，包数 12→**15**（新增 `moodiary_preferences`/`moodiary_media`/`moodiary_editor_host` + `moodiary_data` 升级带状态）。
> - **Phase 0（清扫）✅** — 删 5 个零引用死文件（`diary_type_icon`、`lottie_modal`、`navigator_bar`、`tab_bar`、`data/model/map.dart`，均重新核验零 importer）；删 `webdav_form_sheet` 里过期的「Milestone G+」提示（同步早已上线）。lint dep 漂移（`moodiary_rust`/`moodiary_editor` 的未用 `flutter_lints`）**暂缓**：换 `moodiary_lint` 会 lint 到 FRB 生成的 `lib/src/rust/**`（12 个里 9 个无 ignore 头）+ editor 手写代码，触发大批风格告警，价值低、留待单独处理。baseline 仍 5。
