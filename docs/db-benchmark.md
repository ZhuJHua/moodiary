...
6. 表中不含真实 jieba 分词(Rust FFI)耗时,而**线上导入/重建的一半时间在那里**——
   见下节实测,别用表里的 8.1s 估算真实重建时长。

## 分词耗时(2026-07-28 补测,同机)

上表的分词器是空白切词替身,真实 jieba 的成本单独测在 Rust 侧(`cargo test --release`,
**必须 release**——debug 下同一段分词 4.6ms,release 0.42ms,差 11 倍,用 debug 数会得出
完全错误的结论):

| 场景 | 单篇(2000 字) | 20k 篇折算 |
|---|---|---|
| 逐篇 `tokenize`(每篇一次 FFI 往返,篇内两条线程) | 424 µs → 实测循环 586 µs | 8.5–11.7 s |
| `tokenize_batch`(整批一次过桥,跨篇并行,14 核) | **97.6 µs** | **2.0 s** |

即全量重建 20k 篇的真实耗时约为「表中 8.1s 的库操作 + 分词」,分词部分经批量化后从
约 8.5s 降到约 2s(**6.00x**,同一进程内 A/B 交替测量)。批量路径已接在
`insertDiaries` 与 `rebuildAllIndexes` 上,按 128 篇分块以限制桥载荷与内存峰值。

正文派生(纯文本镜像/媒体/双链)同期从「每项各解析一遍 JSON」改为解析一次多项复用:
三项派生 125.0 → 74.0 µs/篇,两项(`repairData` 的形态)97.9 → 72.0 µs/篇。
