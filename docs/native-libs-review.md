...
- **fast_zip 一度改纯 Dart `archive` 4.2.0（同日晚），次日（2026-09-04）撤回。** 第二节没验的那件事
  验了：宿主 AOT 实测 Stored 写 226 MB/s、读 1.2 GB/s，但 **zip 内 AES 只有 17 / 19 MB/s，且
  `archive` 对加密条目整块进堆**。于是当时把局域网归档从「zip 条目级 AES」改成「明文 zip + 条目走
  `SyncCipher(会话密钥)`」绕开它 —— 这等于**换了线上格式**：2.8.0 已经发出去的构建按旧格式说话，
