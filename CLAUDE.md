...
### Build & Run
`tool/task.dart`'s build/run target the **mobile** app; the editor (`packages/product/moodiary_editor/editor/`) is built automatically as a prerequisite:
```bash
...
```bash
dart tool/task.dart editor           # cd packages/product/moodiary_editor/editor && pnpm install && pnpm build
```
...
moodiary/              # Monorepo root — pub-workspace + Melos coordinator (NO app code here)
  ...
  mobile/              # ── Pure-mobile Flutter app (workspace member, pub name `moodiary`) ──
    pubspec.yaml       #   resolution: workspace; depends on the 8 runtime packages (+ moodiary_lint dev),
    ...
    lib/
      core/            # holdouts only: values/{assistant,diary_type_icon}.dart (rest → moodiary_core)
      component/       # holdouts only: quill_embed/* (legacy Quill) + basic/lottie_modal.dart (rest → moodiary_ui)
      merge/           # one-shot legacy-data migration (incl. MergeUtil.runVersionMigration)
      feature/         # Feature modules (vertical slices; siblings must NOT import each other)
        assistant/     # AI assistant (service in feature/assistant/data/)
        calendar/ diary/ edit/ lock/ media/ setting/ share/ sync/ user/ web_view/
      app/             # Composition layer (aggregates features; may depend on any feature)
        router/        # go_router configuration
        shell/         # Root shell (bottom nav)
        di/            # service_di.dart (business service registration)
      gen/             # Generated assets (l10n moved to the moodiary_l10n package)
  ...
  desktop/             # ── Desktop Flutter app (workspace member, pub name `moodiary_desktop`) ──
    ...
    lib/main.dart  test/
  packages/
    foundation/        # lowest layer (depend on nothing above)
    ...
    product/           # depend on infra/foundation
      moodiary_ui/     #   reusable business-agnostic widgets (component basic + common)
      moodiary_editor/ #   TipTap webview editor widget + local server + Vue source (editor/) + asset
```
...
**foundation** (depend on nothing above):
- **`moodiary_l10n`** — localization (see Localization).
- **`moodiary_rust`** — Rust FFI plugin (see Rust FFI).
- **`moodiary_utils`** — leaf, business-agnostic helpers (file/media/network/logging/color, etc.) **plus the pure-Dart content + migration converters** `tiptap_content.dart`, `quill_to_tiptap.dart` (`QuillDeltaToTiptap`), `markdown_to_tiptap.dart` (`MarkdownToTiptap`). ⚠️ **No domain types here** — utils = generic tools only.
- **`moodiary_models`** — the **domain layer**: 9 Isar `@Collection`s + Freezed DTOs + events, and **domain field enums** (`DiaryType`, `AssistantProviderType`). Generated `.g.dart`/`.freezed.dart` were moved as-is (no `build_runner` re-run) so the Isar schema stays byte-identical → zero DB-migration risk.
...
- **`moodiary_data`** — repositories (`DiaryRepository`, `CategoryRepository`, …) + `DiaryContentUtil`. Kept singleton `.get()` access (data→core is a downward edge, so no constructor injection needed).

**product** (depend on infra/foundation):
- **`moodiary_ui`** — reusable, business-agnostic widgets (`component/basic` + `common`).
- **`moodiary_editor`** — TipTap webview editor (see Diary Editor); decoupled from the app via injection.

**Apps** (`mobile`, `desktop`) sit above all packages and compose them. App-side **holdouts** deliberately NOT pushed into packages: `mobile/lib/core/values/{assistant,diary_type_icon}.dart`, `mobile/lib/component/quill_embed/*` (legacy Quill), `mobile/lib/component/basic/lottie_modal.dart`.

### Layered Dependencies (enforced)

Two layerings:

**1. Cross-package DAG** (foundation → infra → product → apps): upper packages depend on lower, never the reverse. Enforced by **pub's acyclic dependency graph** (a cycle fails resolution); the Melos `categories:` only group/filter. See **Layered Packages** above.
...
(The `data`/`l10n` layers in the checker's table are now mostly empty in-app since that code moved to `moodiary_data`/`moodiary_l10n`; `core`/`component` hold only the few app-side holdouts.) Run via `dart tool/task.dart check-layers` / `analyze` and the `flutter-ci.yml` "Check Layer Dependencies" step; it scans `mobile/lib` only (desktop isn't wired in yet). Known pre-existing violations are listed in `tool/layer_baseline.txt` (a ratchet — it may only shrink; currently 34). To add a temporary exception run `dart tool/check_layers.dart --update-baseline`; the proper fix is to flip the dependency direction.
...
```
feature/<name>/
  application/     # Controllers (Riverpod providers with @riverpod annotation)
...
> `assets/editor/` is a build artifact (gitignored). CI must build the editor (`dart tool/task.dart editor`, or `cd editor && pnpm build`) before building the Flutter app.
...
- `mobile/lib/app/di/service_di.dart`: business services (HttpClient, Assistant, Sync). Lives in the app composition layer because it references feature implementations. The assistant service lives in `feature/assistant/data/`.

Access via `getIt.get<T>()` or `T.get()` static methods on repositories (repositories live in `moodiary_data`).

### Sync System

Supports LAN sync and cloud backup (WebDAV, S3). Key components:
- `SyncRegistry`: Backend registration
