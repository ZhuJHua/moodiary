...
class _TabStyle extends AnimatedWidget {
  ...
  double fixPrecision(double value, int precision) {
  ...
  Widget build(BuildContext context) {
    ...
    double adjustedHeight(
      ...
      if (isSelected) {
        ...
        adjustedLineHeight = (actualLineHeight / 2).ceil() * 2;
      } else {
        // Use floor for unselected state to round down.
        adjustedLineHeight = (actualLineHeight / 2).floor() * 2;
      }

      // Recalculate height as multiplier and fix precision.
      return fixPrecision(adjustedLineHeight / fontSize, 6);
    }

    final double adjustedHeightValue = adjustedHeight(
      textStyle.fontSize!,
      textStyle.height,
      isSelected,
    );
    ...
    return DefaultTextStyle(
      style: textStyle.copyWith(
        color: color,
        fontSize: fixPrecision(textStyle.fontSize!, 6),
        // Fix fontSize precision.
...
class _TabPageSelectorState extends State<TabPageSelector> {
  ...
  void didChangeDependencies() {
    super.didChangeDependencies();
    if (_animation == null ||
        _previousTabController?.animation != _tabController.animation) {
      _setAnimation();
    }
    if (_previousTabController != _tabController) {
      _previousTabController = _tabController;
