...
AutoSyncWatcher                  单开关 autoSync：变更去抖 5s 后 push + 定时轮询 sync（间隔可配，默认 30s）
...
IRemoteSyncBackend               后端抽象（webdav / s3，DI 单注册，可插拔）
        │  readObject / writeObject / deleteObject / statObject
IncrementalSyncEngine            增量引擎（按操作构造）
        │  ├─ runSyncExclusive             进程内互斥：push/pull/re-cipher/归档 同时只跑一个
        ...
        │
SyncManifest (v2)                远端清单：结构化条目 + 媒体清单 + 毫秒时间戳
...
`diary/<id>.json` + `category/<id>.json` + `place/<id>.json` +
...
不放大「读取→写回」基线窗口）：
- **pull**：读 manifest → 条目级 `runPooled` 并发；每条按毫秒时间戳 LWW，
...
2. **LWW 全程比毫秒整数**：`ManifestEntry.timeMs` 是纯 int；引擎所有比较
   ...
   `isAfter` 对象比较 —— 本地 lastModified（SQLite 按微秒存）的微秒尾数会让
   ...
   **删除路径同样走 LWW（日记与分类都要）**：push tombstone 前若远端仍是普通条目
   ...
   对称地，**pull 下载写库失败必须计入 `failed`**（各条目写入抛出的异常由条目级
   catch 统一 `failed++`）—— 否则谎报成功、错误推进 lastSyncTime。
3. **manifest 版本守卫**：`version` 非 2（当前）或 1（2.8.0 遗留，读入后按 v2
   写回）一律抛 `SyncException`（含更高版本 —— 静默丢条目会诱发 manifest
   重建式数据丢失）；单条损坏的 entry 只丢弃该条。
...
- **pull 删除 vs 并发编辑的残余竞态**：长 pull 删除某日记时，若用户恰在
  ...
  按 `lastModified` 的 compare-and-swap 写入（当前仓储的 upsert 无此语义）。
...
测试镜像 `lib/` 结构放在 `test/sync/`。引擎通过端口（`sync_stores.dart`
的 `SyncDiaryStore`/`SyncCategoryStore`/`SyncPlaceStore`/`SyncMediaInfoStore`/`SyncTombstoneStore`/`SyncMediaFiles`）+ 可注入 cipher/logger/
concurrency 解耦，单测全用内存假实现（`sync_test_harness.dart`），不碰数据库 /
文件系统 / Rust / 网络，确定性运行。生产实现只是转发到 repository / `package:file`，
行为不变。

- `data/model/manifest_test.dart`：v2 序列化往返、版本守卫（v1 接受，更旧/更新/缺失拒绝）、
...
- `data/codec_test.dart`（明文路径）、
  `data/sync_provider_scope_test.dart`（多后端 configured 集合 / 切换重注册）、
