...
Dart orchestrates, Rust executes. rig (`rig-core` + `rig-agent`, pinned `=0.42.0`) only adapts the provider protocol and runs the tool loop; every architectural decision — prompt layering, memory, retrieval, the permission gate — lives here. Do not track rig's `main` (it is mid-rewrite around `rig-ecs`); migrate to 0.43 in its own PR from MIGRATING.md.
...
- **System prompt** (`buildStableSystemPrompt`) is ordered sections in one order band: `-100` identity, `-50` guardrails, `-25` retrieval policy (tools only, ≤150 words, test-pinned; one variant with memory on, one telling the model memory is off), `0` the built-in persona, `25` the user's notes (`assistantUserNotes`, ≤2000 chars, omitted when empty), `100` tool catalog. Nothing in it is user-editable except the notes: there are no presets, no custom persona, no per-session tool whitelist. Same inputs must give the same bytes — this is the provider cache prefix. The guardrails' tool sentence is chosen by the permission mode, so each mode has its own stable prefix.
- **Turn context** (`buildTurnContext`) carries only volatile, count-only data: local time, locale, `Saved facts: N`, semantic search on/off. On the OpenAI paths it is a `system` message before the last user message; on Anthropic it is glued onto the last user message, because rig folds mid-conversation system messages onto the end of the `system` array, exactly where `with_prompt_caching()` puts the static breakpoint.
...
- **Events**: `Turn` is emitted per completion call (rig's `CompletionCall`) with `finish_reason`, ids and usage; the `Usage` at `FinalResponse` is the run's aggregate. Context length is the last `Turn`'s `input_tokens` — never the aggregate, which multiplies with every tool call. `finish_reason == length` marks the reply truncated and offers Continue.

## Memory: opt-in and on demand only

`assistantMemoryEnabled` (KV, default on) gates the three memory tools: when off they are not advertised (`toolIdsWithoutMemory`), the retrieval policy says so, and the turn context carries no fact count. When on, every fact is reached through `recallMemory`; nothing is injected into the prompt. Standing preferences belong in the user's notes, written by the user. Diary text never enters `memories`; facts never enter the vector index.
...
The decision is Dart's but the pause is rig's: `ToolObserver.on_tool_call` emits `ToolStarted`, then awaits the Dart `toolGate`; only the exact verdict `run` runs the tool, anything else (a `skip:` reason, a bridge error) becomes `ToolCallAction::Skip` so the tool body never runs and the model reads `Skipped: …`. `ToolApprovalGate` keeps one `Completer` per call id; stop, dispose and a 5-minute timeout all resolve as declined — without that the Rust run would hang forever. rig's tool batch is sequential, so at most one card is ever pending; do not enable `tool_concurrency` while the gate exists. The card sits above the composer on the same glass surface, button radius `MuiRadius.inside(xl, 8)`; typing a reply while a card is pending declines it and queues the text for after the run.
