...
///
/// 只干调色这一件事 —— 色板与生成结果留在调用方页面上。`showMoodiaryAlert` 的卡片
/// 宽度上限 340（内容区约 308），塞不下更多东西，小屏上也会顶到安全区。
Future<Color?> showMoodiaryColorPicker(
  ...
  final confirmed = await showMoodiaryAlert<bool>(
    ...
    actions: [
      ...
      MoodiaryAction(label: context.l10n.ok, value: true, isPrimary: true),
    ],
  );
...
class MoodiarySwatchRow extends StatelessWidget {
  final List<Color> swatches;
  final Color selected;
  final ValueChanged<Color> onSelected;

  const MoodiarySwatchRow({
    super.key,
