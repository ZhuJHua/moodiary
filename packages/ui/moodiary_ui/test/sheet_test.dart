...
void main() {
  ...
  Offset handleOf(WidgetTester tester) {
    final rect = tester.getRect(find.byType(MoodiarySheetScaffold<void>));
    return Offset(rect.center.dx, rect.top - 24);
  }

  group('showMoodiarySheet', () {
    testWidgets('返回被点动作的 value，点遮罩返回 null', (tester) async {
      String? result = 'sentinel';
      await tester.pumpWidget(
        host((context) async {
          result = await showMoodiarySheet<String>(
            context,
            builder: (_) => const MoodiarySheetScaffold<String>(
              title: 'WebDAV',
              actions: [
                MoodiaryAction(label: '取消'),
                MoodiaryAction(label: '保存', value: 'saved', isPrimary: true),
              ],
              child: Text('表单占位'),
            ),
          );
    ...
    testWidgets('动作条在内容之下，且不随内容滚动', (tester) async {
      await tester.pumpWidget(
        host(
          (context) => showMoodiarySheet<void>(
            context,
            builder: (_) => MoodiarySheetScaffold<void>(
              title: '长表单',
              actions: const [MoodiaryAction(label: '保存', isPrimary: true)],
              child: Column(
                mainAxisSize: .min,
                children: [
                  for (var i = 0; i < 40; i++)
        ...
        host(
          (context) => showMoodiarySheet<void>(
            ...
            builder: (_) => const MoodiarySheetScaffold<void>(
              ...
              actions: [
                MoodiaryAction(label: '取消'),
                MoodiaryAction(label: '保存', isPrimary: true),
              ],
  ...
  group('无障碍', () {
    ...
    testWidgets('选中的选项行带 isSelected 标志', (tester) async {
      ...
      await tester.pumpWidget(
        host(
          (context) => showMoodiaryPickerSheet<int>(
            context,
            title: '同步方式',
            selected: 1,
            options: const [
              MoodiarySheetOption(value: 1, label: 'WebDAV'),
              MoodiarySheetOption(value: 2, label: 'S3 / MinIO'),
            ],
          ),
        ),
      );
  ...
  group('showMoodiaryPickerSheet', () {
    testWidgets('点中选项即返回其值，取消返回 null', (tester) async {
      int? picked = -1;
      await tester.pumpWidget(
        host((context) async {
          picked = await showMoodiaryPickerSheet<int>(
            context,
            title: '同步方式',
            selected: 1,
            options: const [
      ...
      await tester.pumpWidget(
        host((context) async {
          picked = await showMoodiaryPickerSheet<int>(
            ...
            title: '同步方式',
            options: const [
              MoodiarySheetOption(value: 1, label: 'WebDAV', enabled: false),
            ],
          );
        }),
      );
  ...
  group('MoodiaryField', () {
    ...
    testWidgets('有内容才出清除键，点了清空', (tester) async {
      ...
      await tester.pumpWidget(
        fieldHost(MoodiaryField(controller: controller, label: '服务器地址')),
      );
    ...
    testWidgets('密码框给眼睛不给清除键，点了切换明文', (tester) async {
      ...
      await tester.pumpWidget(
        fieldHost(
          MoodiaryField(controller: controller, label: '密码', obscureText: true),
        ),
      );
    ...
    testWidgets('禁用时尾部槽位一并消失', (tester) async {
      ...
      await tester.pumpWidget(
        fieldHost(
          MoodiaryField(controller: controller, label: '用户名', enabled: false),
        ),
      );
      expect(find.byIcon(LucideIcons.x), findsNothing);
    });
