...
是空的（`doc` 与 `export` 一起去了 fast_press），层位与闸门保留。内部依赖边只有两条，全部
跨层：`assistant → http`、`sync → http`；foundation 七个 crate 之间零边。

### moodiary_rust —— 一个 .so，五扇门
...
**图片编解码与导出都不在这里**：turbojpeg / libwebp / png 与整条图片管线 2026-09-03 拆去了
