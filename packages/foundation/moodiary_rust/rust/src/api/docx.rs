...
//! 文档。IR 的 Rust 镜像在 `crate::export_ir`，与 PDF 那条链共用一份定义。
//!
...
use crate::export_ir::{IrBlock, IrCell, IrDoc, IrListItem, IrSpan};
...
pub fn write_docx(docs_json: String, style: DocxStyle, out_path: String) -> Result<()> {
    let docs: Vec<IrDoc> =
        serde_json::from_str(&docs_json).context("导出 IR 反序列化失败（Dart 侧 JSON 形状变了？）")?;
    ...
    let file = File::create(&out_path)
        .with_context(|| format!("创建 docx 文件失败：{out_path}"))?;
...
fn numbering_def(id: usize, ordered: bool) -> AbstractNumbering {
    ...
    for level in 0..=MAX_LEVEL {
        let (format, text) = if ordered {
            (NumberFormat::new("decimal"), LevelText::new(format!("%{}.", level + 1)))
        } else {
            // 三种符号轮换，嵌套层级一眼可辨。
            let bullet = ["•", "◦", "▪"][level % 3];
            ...
            let cells: Vec<TableCell> = row
                ...
                .map(|cell| {
                    let mut tc = TableCell::new()
                        .width(column_width * cell.colspan.max(1), WidthType::Dxa);
                    if cell.colspan > 1 {
                        tc = tc.grid_span(cell.colspan);
                    }
                    if cell.rowspan > 1 {
        ...
        if let Some(href) = span.href.as_deref() {
            para = para.add_hyperlink(
                Hyperlink::new(href, HyperlinkType::External)
                    .add_run(styled_run(span, ctx, Some(LINK_COLOR))),
            );
            continue;
        }
...
fn styled_run(span: &IrSpan, ctx: &Ctx, color: Option<&str>) -> Run {
    ...
    run = run.size(half_pt(ctx.style.font_size_pt)).add_text(&span.text);
...
fn image_run(path: &str, width_percent: Option<u32>, ctx: &Ctx) -> Option<Run> {
    ...
    Some(Run::new().add_image(
        Pic::new_with_dimensions(bytes, px_w, px_h).size(final_w, final_h.max(1)),
    ))
}
...
mod tests {
    ...
    fn code_block_is_syntax_highlighted() {
        ...
        let docs = serde_json::json!([{
            "id": "a", "title": "T", "time": "t",
            "blocks": [{"type": "code", "language": "dart",
                        "text": "// 注释\nvoid main() { print(\"x\"); }"}]
        }]);
        write_docx(docs.to_string(), test_style(), out.clone()).expect("导出应当成功");

        let xml = doc_xml(&unzip(&out));
        for (scope, hex) in [("关键字", "D73948"), ("注释", "74747C"), ("字符串", "198810")] {
            assert!(
    ...
    fn unknown_language_falls_back_to_plain() {
        ...
        let docs = serde_json::json!([{
            "id": "a", "title": "T", "time": "t",
            "blocks": [{"type": "code", "language": "没有这种语言", "text": "hello"}]
        }]);
        write_docx(docs.to_string(), test_style(), out.clone()).expect("未知语言也要能导出");
    ...
    fn writes_chinese_content_without_mojibake() {
        ...
        let docs = serde_json::json!([{
            "id": "0190aa11-2222-3333-4444-555566667777",
            "title": "立秋那天的雨",
            ...
            "categoryName": "日常",
            "blocks": [
                {"type": "heading", "level": 2, "spans": [{"text": "早上"}]},
                ...
                {"type": "divider"},
                {"type": "quote", "children": [
                    {"type": "paragraph", "spans": [{"text": "引用一段话"}]}
                ]},
        ...
        assert!(xml.contains("立秋那天的雨"), "标题应当原样落进 document.xml");
    ...
    fn embeds_image_as_media_part() {
        ...
        let docs = serde_json::json!([{
            ...
            "title": "带图",
            "blocks": [
                {"type": "image", "path": jpeg, "widthPercent": 50},
                {"type": "image", "path": "/nowhere/missing.jpg"},
                {"type": "image", "path": "https://example.com/a.png", "external": true}
            ]
        }]);
        ...
        assert_eq!(&bytes[..2], &[0xFF, 0xD8], "媒体内容应当是 JPEG（SOI 魔数）");
        assert!(
        ...
        let docs = serde_json::json!([
            {
                ...
                "blocks": [{"type": "paragraph", "spans": [
                    {"text": "去看", "diaryLinkId": "ccc-ddd"},
                    {"text": "外部目标", "diaryLinkId": "not-in-batch"}
                ]}]
            },
            {"id": "ccc-ddd", "title": "第二篇", "blocks": []}
        ]);

        write_docx(docs.to_string(), test_style(), out.clone()).expect("导出应当成功");

        let xml = doc_xml(&unzip(&out));
        assert!(xml.contains("d_cccddd"), "同批内的双链应当锚到目标日记的书签");
        assert!(
    ...
    fn nested_lists_and_tasks_use_numbering() {
        ...
        let docs = serde_json::json!([{
            ...
            "blocks": [
                {"type": "list", "ordered": false, "items": [
                    {"children": [
                        {"type": "paragraph", "spans": [{"text": "外层"}]},
                        {"type": "list", "ordered": true, "items": [
                            {"children": [{"type": "paragraph", "spans": [{"text": "内层"}]}]}
                        ]}
                    ]}
                ]},
                {"type": "list", "ordered": false, "items": [
                    {"checked": true, "children": [
                        {"type": "paragraph", "spans": [{"text": "买菜"}]}
                    ]},
                    {"checked": false, "children": [
                        {"type": "paragraph", "spans": [{"text": "做饭"}]}
                    ]}
                ]}
            ]
        }]);
        ...
        assert!(xml.contains("☑") && xml.contains("☐"), "任务项用符号表达勾选");
        assert!(
    ...
    fn table_survives_merges_and_headers() {
        ...
        let docs = serde_json::json!([{
            ...
            "blocks": [{"type": "table", "rows": [
                [
                    {"header": true, "colspan": 2, "children": [
                        {"type": "paragraph", "spans": [{"text": "合并表头"}]}
                    ]}
                ],
                [
                    {"children": [{"type": "paragraph", "spans": [{"text": "甲"}]}]},
                    {"children": [{"type": "paragraph", "spans": [{"text": "乙"}]}]}
                ]
            ]}]
        }]);
    ...
    fn multiple_docs_are_separated_by_page_breaks() {
        ...
        let docs = serde_json::json!([
            {"id": "a", "title": "第一篇", "blocks": []},
            {"id": "b", "title": "第二篇", "blocks": []},
            {"id": "c", "title": "第三篇", "blocks": []}
        ]);
    ...
    fn malformed_json_errors_instead_of_panicking() {
        let dir = tempdir();
        let out = dir.join("bad.docx").to_string_lossy().into_owned();
        let err = write_docx("{ not an array }".into(), test_style(), out).unwrap_err();
        assert!(err.to_string().contains("反序列化失败"));
    }
}
