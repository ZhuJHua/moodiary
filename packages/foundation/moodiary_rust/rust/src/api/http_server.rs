//! 应用内通用 HTTP 服务（hyper 1.x，栈由 reqwest 共享）。路由与业务全在 Dart 侧
//! （handler 回调），本模块只负责传输层：
//! - 监听与端口回退（preferred 被占 → OS 随机分配）；
//! - 请求体收集：小 body 内联，超阈值流式落盘（进度回调，移动端不吃内存）；
//! - 响应发送：内存字节，或磁盘文件流式发送并自动支持单段 HTTP Range
//!   （webview `<video>` 依赖 206）。
...
type ProgressFn = Arc<dyn Fn(i64, i64) -> BoxFuture<'static, ()> + Send + Sync>;
...
impl HttpServer {
    ...
    pub async fn start(
        ...
        let handler: HandlerFn = Arc::new(move |req| Box::pin(on_request(req)));
        let progress: ProgressFn = Arc::new(move |received, total| {
            Box::pin(on_body_progress(received, total))
        });
        Self::start_inner(preferred_port, loopback_only, spool_dir, handler, progress).await
    }
    ...
    pub(crate) async fn start_inner(
        ...
        let port = listener.local_addr().context("read local addr failed")?.port();
...
/// 收集请求体：累计超过 [SPOOL_THRESHOLD] 即切换为落盘；每 [PROGRESS_STEP] 与
/// 收尾时各回报一次进度（无 body 的请求不回报）。
...
mod tests {
    ...
    async fn roundtrip_small_body_and_query() {
        ...
        let mut server = HttpServer::start_inner(
            0,
            true,
    ...
    async fn file_response_supports_range() {
        ...
        assert_eq!(suffix.bytes().await.unwrap().as_ref(), &[95, 96, 97, 98, 99]);
    ...
    async fn upload_file_streams_to_server_with_content_length() {
        use crate::api::http::{ClientSettings, HttpClient, HttpMethod};
        ...
        assert!(totals.lock().unwrap().iter().all(|t| *t == payload.len() as i64));
        // 客户端进度单调且收尾于总长
