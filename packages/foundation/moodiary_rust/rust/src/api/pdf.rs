...
pub fn write_pdf(docs_json: String, style: PdfStyle, out_path: String) -> Result<()> {
    let docs: Vec<IrDoc> = serde_json::from_str(&docs_json)
        .context("导出 IR 反序列化失败（Dart 侧 JSON 形状变了？）")?;
...
impl<'a> Markup<'a> {
    ...
    /// 一组 span → 一段 typst 内容表达式。相邻内容用 `+` 相连（typst 的内容可加）。
...
/// 把「只含空白」的 span 并进相邻 span，避免装饰渗到空格上。
///
/// 两条实测规律（探针量的几何，别凭直觉改）：
///
...
/// 把过长的段落按 [PARAGRAPH_CHUNK] 切成多段，切点尽量落在标点之后。
...
mod tests {
    ...
    fn temp(name: &str) -> String {
        let dir = std::env::temp_dir().join(format!("moodiary-typst-{}-{name}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        dir.join(format!("{name}.pdf")).to_string_lossy().into_owned()
    }
    ...
    fn writes_valid_pdf() {
        ...
        let docs = serde_json::json!([{
            ...
            "categoryName": "Daily",
            "blocks": [
                {"type": "heading", "level": 2, "spans": [{"text": "Morning"}]},
                ...
                {"type": "divider"},
                {"type": "quote", "children": [
                    {"type": "paragraph", "spans": [{"text": "quoted"}]}
                ]},
                {"type": "code", "language": "dart", "text": "print(\"hi\");"},
                {"type": "list", "ordered": false, "items": [
                    {"checked": true, "children": [
                        {"type": "paragraph", "spans": [{"text": "done"}]}
                    ]},
                    {"checked": false, "children": [
                        {"type": "paragraph", "spans": [{"text": "todo"}]}
                    ]}
                ]},
                {"type": "table", "rows": [
                    [{"header": true, "colspan": 2, "children": [
                        {"type": "paragraph", "spans": [{"text": "merged head"}]}
                    ]}],
                    [
                        {"children": [{"type": "paragraph", "spans": [{"text": "a"}]}]},
                        {"children": [{"type": "paragraph", "spans": [{"text": "b"}]}]}
                    ]
                ]}
            ]
        }]);

        write_pdf(docs.to_string(), style(), out.clone()).expect("导出应当成功");
    ...
    fn user_text_cannot_inject_typst_code() {
        ...
        for (i, payload) in payloads.iter().enumerate() {
            let docs = serde_json::json!([{
                "id": format!("p{i}"),
                "title": "inject",
                "blocks": [{"type": "paragraph", "spans": [{"text": payload}]}]
            }]);
            write_pdf(docs.to_string(), style(), out.clone())
    ...
    fn huge_paragraph_completes() {
        ...
        let docs = serde_json::json!([{
            "id": "huge",
            "title": "长文本测试",
            "blocks": [{"type": "paragraph", "spans": [{"text": text}]}]
        }]);

        let started = std::time::Instant::now();
        write_pdf(docs.to_string(), style(), out.clone()).expect("32 万字应当能导出");
    ...
    fn missing_font_fails_loudly() {
        ...
        let docs = serde_json::json!([{"id": "x", "title": "x", "blocks": []}]);
        let err = write_pdf(docs.to_string(), bad, out.clone()).unwrap_err();
    ...
    fn missing_image_is_skipped() {
        ...
        let docs = serde_json::json!([{
            "id": "img",
            "title": "with missing image",
            "blocks": [
                {"type": "image", "path": "/nowhere/gone.jpg"},
                {"type": "paragraph", "spans": [{"text": "after"}]}
            ]
        }]);
        write_pdf(docs.to_string(), style(), out.clone()).expect("缺图不应让导出失败");
        assert_eq!(&read_head(&out, 5), b"%PDF-");
    }
}
...
mod whitespace_between_marks {
    ...
    /// 渲染一段 spans，返回 (拼接后的文字, 所有横线的长度)。
    /// 删除线 / 下划线在 typst 的输出里就是 `Geometry::Line`。
    fn render(spans: serde_json::Value) -> (String, Vec<f64>) {
        let docs: Vec<IrDoc> = serde_json::from_value(serde_json::json!([{
            "id": "a", "title": "", "time": "t",
    ...
    /// 单独量一个词在当前字体字号下的宽度，用来和装饰线长做对比。
    fn word_width(word: &str) -> f64 {
        let docs: Vec<IrDoc> = serde_json::from_value(serde_json::json!([{
            "id": "a", "title": "", "time": "t",
            "blocks": [{"type": "paragraph", "spans": [{"text": word, "underline": true}]}]
        }]))
        .unwrap();
        let font_blob = std::fs::read(super::tests::font_path()).unwrap();
        let st = super::tests::style();
    ...
    fn strike_does_not_bleed_into_following_link() {
        let (text, lines) = render(serde_json::json!([
            {"text": "AAA", "strike": true},
            {"text": " "},
            {"text": "BBB", "href": "https://example.com"}
        ]));
    ...
    fn both_sides_marked_falls_back_to_spacer() {
        let (_text, lines) = render(serde_json::json!([
            {"text": "AAA", "strike": true},
            {"text": " "},
            {"text": "BBB", "underline": true}
        ]));
