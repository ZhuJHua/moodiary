...
`pdf.rs` / `docx.rs` 是引擎，只收纯闭包），与 fast_image 同一套形状。

- **foundation 叶子包，零 `moodiary_*` 依赖，只有 `moodiary_export` 能依赖它**
  （`tool/check_layers.dart` 的 `_nativePkgOwners`）。
...
- 依赖收窄的实测结论（别再重推）：`image` 的六个 feature（bmp/gif/jpeg/
...
- 体积：typst 是这个库的大头（`opt-level` 档位实测 38.86 / 18.51 MiB）。
...
- **typst 走自家 fork**（`https://github.com/ZhuJHua/typst`，分支 `moodiary/slim`，基于 v0.15.1 tag）。
  ...
  **升级 typst**：在 fork 上 rebase 那一个提交（`git log v0.15.1..moodiary/slim`），推上去改两处 rev
  ...
  feature 合并会把默认全开回来，`cargo tree` 看着 patch 生效其实一个都没关。
