...
（`hf.rs`，ONNX 推理侧；WordPiece / SentencePiece / BPE 通吃）。自带 FRB（入口类 `FastTextLib`）
与原生库 **libfasttext**。2026-09-03 从 moodiary_rust 的 `text` + `hf_tokenizer` 两个 crate 拆出来。

- 多个消费方（data / diary / assistant / ml），不进 `_nativePkgOwners`。
- **启动时装载**（`main.dart` 的 `FastText.ensureInitialized()`）：搜索索引、迁移、心情建议都要它，
  不做延迟；`ensureInitialized` 仍是幂等的，ml 的两个后端在 `HfTokenizer.fromFile` 前也各 await 一次。
- **宿主测试用 `testing.dart`**：`installFakeFastText(tokenize)` 经 `FastTextLib.initMock` 只桩掉
